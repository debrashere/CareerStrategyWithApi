...
function renderUserSkills(data) {
  ...
  if (userProfile.skills){
      userProfile.skills.map( function(skill, index) {          
      skills +=
...
function displayCareerStrategyResults(data) { 
  ...
  if (!data || !data.userProfile || data.userProfile.length == 0) {
      ...
      $('.js-user-details ').prop("hidden", true);           
...
function displaySkillsMasterList(data) {
  ...
  data.skill.map( function(skill, index) {           
      skills +=  `<a href=# id="masterSkill${index}" class="master-skill-link js-master-skill">${skill.skill}</a>`;     
     });
     skills += '</p>';    
...
function renderJobProspects(data) {
    ...
    $( ".js-section-job-prospects-details" ).html('');
    ...
    data.prospect.map( function(prospect, index) {  
      ...
      prospects +=
       `<div class="flex-item-job-prospect">
            <div class="section-header"><span  tabindex="0" id="prospect${index}">${prospect.what}</span>  <br />
                    <a id="ProspectEdit-${index}" href=# class="js-edit-prospect"><img alt="edit prospect" src="./images/icon-edit.png" />  (edit)</a>   
                    <a id="ProspectDelete-${index}" href=# class="js-delete-prospect"><img alt="delete prospect" src="./images/icon-delete.png" /> (delete)</a>  </div>
    ...
    $('.js-section-job-prospects-details').append(`${prospects}`);  
...
/*
  Executed when user clicks the cancel logout link at top of page
  Will delete local storage settings including auth key.
*/
function watchLogoutClick() { 
  $('.js-nav-logout').click(event => {
    event.preventDefault();
